...
impl Peon{
    pub fn new(f: i16, c: i16, color_ingresado: Color) -> Self{
        Peon{fila: f, columna: c, color: color_ingresado}
    }
}
...
impl AnalisisAtaque for Peon{
    ...
    fn puedo_atacar_enemigo(&self, posicion_contrincante: (i16, i16)) -> bool{
        ...
        if self.color == Color::Negro{
            (self.columna - posicion_contrincante.1).abs() == 1 && posicion_contrincante.0 - self.fila == 1
        }else if self.color == Color::Blanco{
            (self.columna - posicion_contrincante.1).abs() == 1 && self.fila - posicion_contrincante.0 == 1
        }else{
            println!("asodiaosd");
            std::process::exit(-1);
        }
